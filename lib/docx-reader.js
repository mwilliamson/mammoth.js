...
function read(options) {
    ...
    if (options.path) {
        ...
        docxFile = openZipStream(fs.createReadStream(options.path));
    } else if (options.file) {
        docxFile = options.file;
    } else if (options.uint8Array) {
        var buffer = new Buffer(options.uint8Array);
        var stream = new streamBuffers.ReadableStreamBuffer();
        stream.put(buffer);
        docxFile = openZipStream(stream);
    } else {
