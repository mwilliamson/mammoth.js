...
function DocumentConversion(options, comments) {
    ...
    function convertCommentReference(reference, messages) {
        return findHtmlPath(reference, htmlPaths.ignore).wrap(function() {
            ...
            if (commentCountByInitials[comment.authorInitials] === undefined) {
                commentCountByInitials[comment.authorInitials] = 0;
            }
            var count = ++commentCountByInitials[comment.authorInitials];
            var label = "[" + comment.authorInitials + count + "]";
