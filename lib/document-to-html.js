...
var bodyReader = require('./docx/body-reader')
...
function DocumentConversion(options, comments) {
    ...
    function convertParagraph(element, messages, options) {
        return htmlPathForParagraph(element, messages).wrap(function() {
            ...
            if (element.colCount){
				content = content.concat(Html.text(element.colCount));
			}

			if (element.footerReferences && element.footerReferences.length > 0) {
				element.footerReferences.forEach( function(footer) {
					content = content.concat(deferredConversion(createDeferredFooter)(footer.value, messages, options));
				});
			}
    ...
    function htmlPathForParagraph(element, messages) {
		...
		if ( element.styleId && (element.styleId.startsWith('Bullet') || element.styleId.startsWith('bullet')) ) {
			return htmlPaths.topLevelElement("pwl:bullet");
        }
        ...
        if (style) {
            ...
            if ( element.styleId ) {
				...
				if ( ['IssueTitle'].includes( element.styleId ) ) {
					style = htmlPaths.topLevelElement("pwl:title");
				}

				// Services/Products - Child Term
				if ( ['Product'].includes( element.styleId ) ) {
					style = htmlPaths.topLevelElement("pwl:product");
				}

				if ( ['PageHeader'].includes( element.styleId ) ) {
					style = htmlPaths.topLevelElement("h3");
				}

                if ( ['GraphTitle'].includes( element.styleId ) ) {
                    style = htmlPaths.topLevelElement("h4");
                }
            }

            if ( element.colCount ) {
                style = htmlPaths.topLevelElement("pwl:directive", {'colCount': element.colCount});
            }
    ...
    function createDeferredChart(element, messages, options){
        return element.read().then(
            function (value) {
                return Html.freshElement('pwl:chart', null, [Html.text(JSON.stringify(value))]);
            }
        );
    }
	function createDeferredFooter(element, messages, options) {
		return element.read().then(
			function (value) {
                let footer_element = bodyReader.createBodyReader(element.options).readXmlElement(value);
				return Html.freshElement('pwl:footer', null, convertElements(footer_element.value, messages, options));
			}
		);
	}
    ...
    var elementConverters = {
        ...
        "chart": deferredConversion(createDeferredChart),
        "footer": deferredConversion(createDeferredFooter),
        "table": convertTable,
