...
function DocumentConverter(options) {
    ...
    var elementConverters = {
        "document": function(document, html, messages, callback) {
            convertElements(document.children, html, messages, function(error) {
                if (error) {
                    ...
                    convertElements(document.footnotes, html, messages, function(error) {
                        if (error) {
                            callback(error);
                        } else {
                            html.close();
        ...
        "footnoteReference": function(element, html, messages, callback) {
            var number = element.footnoteNumber.toString();
            html.open(htmlPaths.element("sup"));
            html.open(htmlPaths.element("a", {href: "#footnote-" + number}));
            html.text(element.footnoteNumber.toString());
            html.close();
            html.close();
            callback();
        },
        "footnote": function(element, html, messages, callback) {
            html.open(htmlPaths.element("li", {id: "footnote-" + element.footnoteNumber}));
