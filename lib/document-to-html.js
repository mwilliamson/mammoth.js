...
function DocumentConversion(options) {
    ...
    var elementConverters = {
        ...
        "hyperlink": function(element, html, messages, callback) {
            var href = element.anchor ? "#" + element.anchor : element.href;
        ...
        "bookmarkStart": function(element, html, messages, callback) {
            if (element.name === "_GoBack") {
                lastEditBookmardId = element.id;
                return callback();
            }

            html.open(htmlPaths.element("span", {
                id: element.name,
                "data-bookmark-start": element.id
            }), {alwaysWrite: true});
            html.close();
            callback();
        },
        "bookmarkEnd": function(element, html, messages, callback) {
            if (element.id === lastEditBookmardId) {
                return callback();
            }

            html.open(htmlPaths.element("span", {
                "data-bookmark-end": element.id
            }), {alwaysWrite: true});
            html.close();
            callback();
        }
