...
function DocumentConversion(options, comments) {
        ...
        if (style) {
            ...
            if ( element.styleId ) {
                ...
                if ( ['Product'].includes( element.styleId ) ) {
                    style = htmlPaths.topLevelElement("h2");
                }

                if ( ['IssueTitle', 'PageHeader'].includes( element.styleId ) ) {
    ...
    function createDeferredChart(element, messages, options){
        return element.read().then(
            function (value) {
                return Html.freshElement('pwl:chart', null, [Html.text('Chart')]);//[Html.text(JSON.stringify(value))]);
            }
        );
    }
