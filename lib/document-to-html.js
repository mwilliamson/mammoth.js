...
var beautifyHtml = require("js-beautify/js/lib/beautify-html")
...
function DocumentConverter(options) {
    ...
    function convertToHtml(document) {
        ...
        return promises.nfcall(elementToHtml, document, html, messages).then(function() {
            html.closeAll();
            var htmlString = html.asString();
            if (options.prettyPrint) {
                htmlString = beautifyHtml.html_beautify(htmlString, {indent_size: 2});
            }
            return new results.Result(htmlString, messages);
        });
    }
