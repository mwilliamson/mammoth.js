...
function elementEmptier(element) {
    ...
    // never empty table cells
    if ( ['td', 'tr'].includes(element.tag.tagName)){
        tableDepth += 1;
        let children = removeEmpty(element.children);
        let node = [ast.elementWithTag(element.tag, children)];
        tableDepth -= 1;
        return node;
    }

    if ( tableDepth > 0 && ['p'].includes(element.tag.tagName)){

        var children = removeEmpty(element.children);
        if (children.length === 0 && !ast.isVoidElement(element)) {
            // prevent either mammoth or gutenberg from stripping empty table cells
            return [ast.elementWithTag(htmlPaths.element('span', {style: 'display:none;'}, {fresh: false}), [ast.text('empty cell')])];
        } else {
