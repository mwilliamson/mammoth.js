...
function props(obj) {
    ...
    entries.forEach(function(entry) {
        Promise.resolve(entry[1]).then(
            function(resolvedValue) {
                result[entry[0]] = resolvedValue;
                resolvedValueCount += 1;

                if (resolvedValueCount === entries.length) {
    ...
    return addAlsoToPromise(deferred.promise);
}
