...
var readXml = require("./office-xml-reader").readXmlFromZipFile;
...
function BodyReader(options) {
    ...
    function readSectionProperties(element) {
        var sectPrElement = element.first('w:sectPr');
        if ( sectPrElement ){

            var colCount = sectPrElement
                .first("w:cols")
                .attributes["w:num"];

			var rawFooterReferences = sectPrElement.getElementsByTagName("w:footerReference");
			var footerReferences = []
            if (rawFooterReferences.length > 0) {
            	footerReferences = rawFooterReferences.map(readFooter);
			}
            return {
				colCount: colCount ? colCount : 1,
				footerReferences: footerReferences ? footerReferences : null
            };
        }
        ...
        "w:pPr": function(element) {
            return readParagraphStyle(element).map(function(style) {
                return {
                    ...
                    sectPr: readSectionProperties(element)
                };
            });
		},
    ...
    function readFooter(element) {

        var properties = element.attributes;

        var footerPath = 'word/' + relationships.findTargetByRelationshipId(properties['r:id']);
        return elementResult(documents.Footer({
            footerType: properties['w:type'],
            readFooter: readXml.bind(null, docxFile, footerPath),
            options: options
        }));
    }
    ...
    function readDrawingElement(element) {
        ...
        var charts = element
            .getElementsByTagName("a:graphic")
            .getElementsByTagName("a:graphicData")
            .getElementsByTagName("c:chart");
        if ( charts.length > 0) {
            return combineResults(charts.map(readChart.bind(null, element)));
        }
    ...
    function readChart(element, chart) {
        ...
        var chartPath = 'word/' + relationships.findTargetByRelationshipId(chart.attributes['r:id']);

        return elementResult(documents.Chart({
            id: properties['id'],
            name: properties['name'],
            readChart: readXml.bind(null, docxFile, chartPath),
        }));
    }
