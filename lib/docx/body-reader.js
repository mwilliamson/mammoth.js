...
function BodyReader(options) {
    ...
    function readRunProperties(element) {
        return readRunStyle(element).map(function(style) {
            ...
            return {
                ...
                isHighlight: readHighlight(element.first('w:shd'), element.first('w:highlight')),
    ...
    function readHighlight(shadowelement, highlightelement) {
        var highlight = false;
        [shadowelement, highlightelement].forEach(function(element) {
            if (element) {
                var value = element.attributes["w:val"];
                highlight = highlight || value !== "false" && value !== "0" && value !== "none";
            }
        });
        return highlight;
    }
