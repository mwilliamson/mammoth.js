...
function BodyReader(options) {
    ...
    function readRunProperties(element) {
        return readRunStyle(element).map(function(style) {
            ...
            return {
                ...
                isOverline: readOverline(element.first('w:shd'), element.first('w:highlight')),
    ...
    function readOverline(shadowelement, highlightelement) {
        var overline = false;
        [shadowelement, highlightelement].forEach(function(element) {
            if (element) {
                var value = element.attributes["w:val"];
                overline = overline || value !== "false" && value !== "0" && value !== "none";
            }
        });
        return overline;
    }
