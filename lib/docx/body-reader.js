...
function BodyReader(options) {
    ...
    var xmlElementReaders = {
         ...
         * symbol usage
         */
        "w:sym": function(element) {
            var elementValue = element.attributes["w:char"].toLowerCase();
            var i;
            for (i = 0; i < codeArray.length; i++) {
                if (elementValue.search(codeArray[i].symbolCodeValue) != -1) {
                    elementValue = elementValue.replace(new RegExp(codeArray[i].symbolCodeValue, 'g'), codeArray[i].unicodeValue);
                }
            }
            var hexes = elementValue.match(/.{1,4}/g) || [];
            var back = "";
            for (i = 0; i < hexes.length; i++) {
                back += String.fromCharCode(parseInt(hexes[i], 16));
            }
            return elementResult(new documents.Text(back));
        },
