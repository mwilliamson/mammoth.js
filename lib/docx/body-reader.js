...
function BodyReader(options) {
    ...
    var xmlElementReaders = {
        ...
        "w:sym": function(element) {
            var value = element.attributes["w:char"].toLowerCase();
            var i;
            for(i=0;i<codeArray.length;i++){
                if (value.search(codeArray[i].symbolCodeValue) != -1) {
                    var value = value.replace(new RegExp(codeArray[i].symbolCodeValue, 'g'), codeArray[i].unicodeValue);
                }
            }
