...
function endingsTableCase(endingIfTable, endingIfNotTable) {
    return tableState.isTable ? endingIfTable : endingIfNotTable;
}
...
function markdownParagraph(){
    return {start: "", end: endingsTableCase(" ", "\n\n")};
}
...
function markdownHeader(index) {
    return function() {
        return {
            start: endingsTableCase("", repeatString("#", index) + " "),
            end: endingsTableCase(" ", "\n\n")
        };
    };
}
    ...
    return function() {
        return {
            start: function() {
                ...
                var start = endingsTableCase("", hasParentList ? "\n" : "");
                listState.list = {
                ...
                return endingsTableCase("", hasParentList ? "" : "\n");
            }
        };
    };
}
...
function markdownListItem() {
    ...
    var start = endingsTableCase("", repeatString("\t", list.indent) + bullet + " ");
    ...
    return {
        start: "",
            ...
            if (tableState.trCount === 0) {
                end += "|" + new Array(tableState.tdCount).fill("-|").join("") + "\n";
            }
            tableState.trCount++;
            tableState.tdCount = 0;
            return end;
