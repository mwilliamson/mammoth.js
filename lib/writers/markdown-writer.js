...
function markdownList(attributes, list) {
    return {
        start: list.indent > 0 ? "\n" : "",
        end: list.indent > 0 ? "" : "\n"
    };
...
function markdownListItem(attributes, list) {
    list.count++;
    var start = repeatString("\t", Math.max(list.indent, 0)) +
        (list.type === "ul" ? "-" : list.count + ".") + " ";
...
function markdownWriter() {
    ...
    function open(tagName, attributes) {
        ...
        elementStack.push({end: element.end, list: parentElementList});
